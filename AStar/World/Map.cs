namespace AStar.World
{
    public class Map
    {
        ...
        public IEnumerable<(Tile Neighbor, int Cost)> ExpandTile(Tile referenceTile, int horizontalCost, int verticalCost)
        {
            if(referenceTile.X > 0)
            {
                if (_tiles[referenceTile.X - 1][referenceTile.Y].Passable)
                {
                    yield return (_tiles[referenceTile.X - 1][referenceTile.Y], horizontalCost);
                }
            }
            if (referenceTile.Y > 0)
            {
                ...
                {
                    yield return (_tiles[referenceTile.X][referenceTile.Y - 1], verticalCost);
                }
            }
